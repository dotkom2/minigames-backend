...
async function createScore(req, res) {
  try {
    const { userName, game_id, score, game_round } = req.body;
    ...
    // Create a new score
    const newScore = new Scores({ userName, game_id, score, game_round });
    await newScore.save();
...
async function findHighestScore(req, res) {
  try {
    const { game_id, game_round } = req.body;

    if (!game_id) {
      return res.status(400).json({ message: 'game_id parameter is required' });
    }

    const highestScore = await Scores.findOne({ game_id, game_round }).sort({ score: -1 });
...
async function findScores(req, res) {
  try {
    const { userName, game_round } = req.body;
    ...
    const userScores = await Scores.find({ userName, game_round });
    const gameIdsAndScores = [];

    for (const score of userScores) {
      const game = await Games.findOne({ game_id: score.game_id });

      const highestScore = await Scores.findOne({ game_id: score.game_id, }).sort({ score: -1 });
      let isWinner = false
      if (highestScore.userName === user.userName) {
        isWinner = true
      }
      if (game) {
...
async function findScoresByGameId(req, res) {
  try {
    const { game_id, game_round } = req.body;
    console.log("DATA =======", req.body)
    const games = await Scores.find({ game_id, game_round }).sort({ score: -1 })
